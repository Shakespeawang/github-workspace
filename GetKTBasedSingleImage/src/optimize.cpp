...
void pointsInRect(const std::vector<cv::Point2d> & imagePts_InImage,const std::vector<cv::Point3d> & objectPts_InImage,	const cv::Rect rect, std::vector<cv::Point2d> & imagePts_NearPP, std::vector<cv::Point2d> & objectPts_NearPP)
{
	...
	for (size_t i = 0; i < imagePts_InImage.size(); i++)
	{
		if (imagePts_InImage[i].x >= rect.x && imagePts_InImage[i].x <= rect.x + rect.width
			&& imagePts_InImage[i].y >= rect.y && imagePts_InImage[i].y <= rect.y + rect.height){
			imagePts_NearPP.push_back(imagePts_InImage[i]);
			objectPts_NearPP.push_back(cv::Point2d(objectPts_InImage[i].x, objectPts_InImage[i].y));
		}
	}
}
...
struct ReprojectionError {
	...
	bool operator()(const T * const K, const T * const R, const T * const t, const T * const D, T * residual) const
	{
		...
		T r2 = x * x + y * y;
		T k_p = T(1.0) + D[0] * r2 + D[1] * r2 * r2 + D[4] * r2 * r2 * r2;
...
bool imageRun(const cv::Mat & realImg, const cv::Size boardSize, const cv::Size squareSize, const cv::Point2d principalPt, double & focalLength, cv::Mat & rotMatrix, cv::Mat & tranVect, cv::Mat & dist, double f_bound[2], double k1_bound[2], double k2_bound[2])
{
	...
	if (getCornerPoints(imagePts_InImage, objectPts_InImage, realImg, boardSize, squareSize)){
		// step 2: runOptimize
		cv::Mat K_init, rotMatrix_init, tranVect_init, dist_init = dist;
		cv::Mat K_opt;
		runOptimize(imagePts_InImage, objectPts_InImage, principalPt, K_init, rotMatrix_init, tranVect_init, dist_init, K_opt, rotMatrix, tranVect, dist, f_bound, k1_bound, k2_bound, cv::Rect(realImg.cols / 4, realImg.rows / 4, realImg.cols / 2, realImg.rows / 2));
