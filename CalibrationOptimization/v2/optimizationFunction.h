...
using namespace cv;
 ...
 bool readParams(std::string path, int nums, std::vector<cv::Mat_<float>> & vec1, cv::Size Size1, std::string flag1, std::vector<cv::Mat_<float>> & vec2, cv::Size Size2 = cv::Size(0, 0), std::string flag2 = "");
 ...
 cv::Mat readImage(int _i, int _j, int _k , std::string path /*= IN_PATH*/, std::string suffix = ".png", std::string connector = "-");
 bool writeImage(cv::Mat image, int _i, int _j, int _k, std::string path /*= OUT_PATH*/, std::string suffix = ".png", std::string connector = "-");
 ...
 bool out_all_point(const vector < vector < vector < Point3f > > > & all_observe, const string ss1 /*= OUT_PATH + "∆Â≈Ã∏ÒΩ«µ„.txt"*/);
